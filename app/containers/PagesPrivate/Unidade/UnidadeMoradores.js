...
import { saveMorador, moradorDelete } from 'services/MoradorService';
import { getMoradores } from 'services/UnidadeService';
import IconButton from '@material-ui/core/IconButton';
import { DateTime, Settings } from 'luxon';
import DeleteIcon from '@material-ui/icons/Delete';
import Button from '@material-ui/core/Button';
import CadastroDlg from 'components/CadastroDlg';
import AddMoradorForm from './AddMoradorForm';
...
const UnidadeMoradores = props => {
  ...
  async function handleDeleteDlgYes() {
    await moradorDelete(dataDelete.id);
  ...
  async function handleSalvarAdd(values, form) {
    ...
    const response = await saveMorador(morador);
    if (tableRef.current) tableRef.current.onQueryChange();
  ...
  const columns = [
    ...
    { title: 'Cadastrado em', field: 'created_at', render: rowData => (DateTime.fromISO(rowData.createdAt).toFormat('dd/MM/yyyy HH:mm:ss')) },
    {
      ...
      render: rowData => (
        <React.Fragment>
          <Button size="small" onClick={() => (handleClickOpenDelete(rowData))} className={classes.btnAction}>
            <DeleteIcon />
            {' '}
Excluir
          </Button>
    ...
    <>
      ...
      <DataTable
        ...
        data={query => new Promise((resolve, reject) => {
          getMoradores(unidadeId, query).then(result => {
            resolve({
...
export default withStyles(styles)(UnidadeMoradores);
