...
import { saveVisitante, deleteVisitante } from 'services/VisitanteService';
import { getVisitantes } from 'services/UnidadeService';
...
const UnidadeVisitantes = props => {
  ...
  async function handleDeleteDlgYes() {
    await deleteVisitante(dataDelete.id);
  ...
  async function handleSalvarAdd(values, form) {
    ...
    const response = await saveVisitante(visitante);
    if (tableRef.current) tableRef.current.onQueryChange();
...
Excluir
  ...
  return (
    <>
      ...
      <CadastroDlg
        open={openAdd}
        dialogTitle="Adicionar Visitante"
      ...
      <DataTable
        ...
        data={query => new Promise((resolve, reject) => {
          getVisitantes(unidadeId, query).then(result => {
            resolve({
