...
//import MaterialTable from 'material-table'
import EditarIcon from '@material-ui/icons/Description';
  ...
  const columns = [
    ...
    { title: "Apto", field: "nome", type: "string", editable: "onAdd", filterPlaceholder: 'Apartamento' },
    {
      ...
      render: rowData => {
        return (
          <React.Fragment>
            <Button color="primary" size="small" onClick={() => {history.push(`/unidade/${rowData.id}`)}} className={classes.btnEditar}>
              <EditarIcon />&nbsp;Editar
            </Button>
          </React.Fragment>
        );
...
<IconButton onClick={() => {history.push(`/unidade/${rowData.id}`)}} className={classes.btnAction} aria-label="Delete"  >
